...
import com.unittest.item.ModItem;
...
public class Item {
	...
	private static final IItemPropertyGetter DAMAGED_GETTER = (ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) -> stack.isItemDamaged() ? 1.0F : 0.0F;
	private static final IItemPropertyGetter DAMAGE_GETTER = (ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) -> MathHelper.clamp((float)stack.getItemDamage() / (float)stack.getMaxDamage(), 0.0F, 1.0F);
	private static final IItemPropertyGetter LEFTHANDED_GETTER = (ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) -> entityIn != null && entityIn.getPrimaryHand() != EnumHandSide.RIGHT ? 1.0F : 0.0F;
	private static final IItemPropertyGetter COOLDOWN_GETTER = (ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn) -> entityIn instanceof EntityPlayer ? ((EntityPlayer)entityIn).getCooldownTracker().getCooldown(stack.getItem(), 0.0F) : 0.0F;
	...
	protected RayTraceResult rayTrace(World worldIn, EntityPlayer playerIn, boolean useLiquids) {
		...
		float f6 = f3 * f4;
		float f7 = f2 * f4;
		Vec3d vec3d1 = vec3d.addVector(f6 * 5.0D, f5 * 5.0D, f7 * 5.0D);
