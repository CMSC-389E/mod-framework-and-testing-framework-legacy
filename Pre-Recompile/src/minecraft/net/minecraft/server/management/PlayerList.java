...
public abstract class PlayerList
{
    ...
    public void initializeConnectionToPlayer(NetworkManager netManager, EntityPlayerMP playerIn)
    {
        ...
        try {
        	...
        	String result = playerIn.getName()+"\t"+playerIn.getEntityWorld().getWorldInfo().getWorldName();
        	String result2 = playerIn.getEntityWorld().getWorldInfo().getWorldName();
        	File f = Paths.get("saves", result2, ".info").toFile();
//        	File f = new File("saves\\"+result2+"\\.info");
        	if (f.exists()) {
            	List<String> results = Files.readAllLines(Paths.get("saves",result2,".info"));
            	found = new HashSet<String>(results);
        	}

			FileOutputStream out = new FileOutputStream(Paths.get("saves",result2,".info").toFile(),true);
			
			if (!found.contains(result)) out.write((result + "\n").getBytes());
			out.close();
		} catch (IOException e) {
