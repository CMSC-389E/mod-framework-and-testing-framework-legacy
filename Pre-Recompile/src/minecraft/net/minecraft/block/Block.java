...
import com.unittest.block.ModBlock;
...
public class Block {
	...
	protected static void registerBlock(int id, String textualID, Block block_) {
	...
	public static void registerBlocks() {
		...
		for(Block block16 : REGISTRY)
			if(set.contains(block16))
				...
				UnmodifiableIterator<IBlockState> unmodifiableiterator = block16.getBlockState().getValidStates().iterator();
	...
	protected void dropXpOnBlockBreak(World worldIn, BlockPos pos, int amount) {
		if(!worldIn.isRemote && worldIn.getGameRules().getBoolean("doTileDrops"))
			while(amount > 0) {
				...
				amount -= i;
				worldIn.spawnEntity(new EntityXPOrb(worldIn, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, i));
			}
	}
